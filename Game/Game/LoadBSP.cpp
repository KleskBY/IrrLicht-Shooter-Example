...
void PrintShader(const quake3::IShader* shader)
{
	stringc s;
	quake3::dumpShader(s, shader, false);
	printf(s.c_str());
}
...
std::vector<scene::SMesh*> SplitMeshBuffer(const scene::IMeshBuffer* buffer, u32 maxVerts)
{
...
bool LoadLevelBsp8(std::string levelName)
{
	...
	//Addition (shader) items
	scene::IMesh* additional_mesh = q3mesh->getMesh(quake3::E_Q3_MESH_ITEMS); // the additional mesh can be quite huge and is unoptimized
	ScaleMeshVertices(additional_mesh, 0.025f);
	for (u32 i = 0; i != additional_mesh->getMeshBufferCount(); ++i)
	{
		const IMeshBuffer* meshBuffer = additional_mesh->getMeshBuffer(i);
		const video::SMaterial& material = meshBuffer->getMaterial();
		const s32 shaderIndex = (s32)material.MaterialTypeParam2; // The ShaderIndex is stored in the material parameter
		const quake3::IShader* shader = q3mesh->getShader(shaderIndex); // the meshbuffer can be rendered without additional support, or it has no shader
		if (!shader) continue;
		auto node = smgr->addQuake3SceneNode(meshBuffer, shader);
	}

	//Entity list (brush entities)
	irr::scene::quake3::tQ3EntityList& entityList = q3mesh->getEntityList();
	for (u32 e = 0; e != entityList.size(); ++e)
	{
		scene::IMesh* brushMesh = q3mesh->getBrushEntityMesh(e);
		if (!brushMesh) continue;
		if (!brushMesh->getMeshBufferCount()) continue;
		ScaleMeshVertices(brushMesh, 0.025f);
...
bool LoadLevelBsp(std::string levelName)
{
	...
	collision::AddCollisionForMesh(node, mesh->getMesh(0));
	return true;
}